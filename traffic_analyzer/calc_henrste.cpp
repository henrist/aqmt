...
std::ofstream* openFileW(std::string filename, std::string folder)
{
    ...
    //	std::cout << "opened " << filename_out << std::endl;
    return f;
}
...
{
//	std::cout << "Processing drops and marks data ... " << std::endl;
...
double variance(std::vector<double> *samples, double *avg_res, double *p99_res, double *p1_res, double *p25_res, double *p75_res) {
    ...
    *p99_res=0;
    ...
    if (flowsamples > 1)
        var = (((double(flowsamples) * sumsq) - (tot * tot))/
		(double(flowsamples) * double(flowsamples-1)));
    avg = tot/flowsamples;
  	*avg_res = avg;
    return var;
}
